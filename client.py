...
def capture_packets(interface='Ethernet', port_range=(20200, 20300)):
    ...
    dispFilter = (f'ip.src == {local_ip} '
                  f'and tcp.dstport >= {port_range[0]} '
                  f'and tcp.dstport <= {port_range[1]} '
                  f'and tcp.payload > 1')
    print(dispFilter)
    packet_data = b""
    try:
        ...
        for packet in capture.sniff_continuously():
            ...
            with open(f'{os.getcwd()}\\logs_client\\buffer.txt', 'a+') as file:
            ...
            if len(packet_data) >= packet_length:
                if parse_all(packet_data, proto_type, packet_length):
                    ...
                    log(str(packet.sniff_time), 'logs_client', packet_data)
                    with open(f'{os.getcwd()}\\logs_client\\buffer.txt', 'w+') as file:
                        file.write(packet_data[packet_length:].hex())
                else:
                    log(str(packet.sniff_time)+'_failed', 'logs_client', packet_data)
...
if __name__ == "__main__":
    print('These are the client-side packets\n')
    capture_packets()